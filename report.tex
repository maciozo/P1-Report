...
Using equation~\ref{eq:ff}, we can calculate the fill factor of the solar cell to be 0.612. Given an exposed cell area of \SI{1}{\centi\metre\squared}, light source power density ($\rho$) of \SI{0.35}{\kilo\watt\per\metre\squared}, and a distance between the light and cell of \SI{10}{\centi\metre}, the power conversion efficiency ($\eta$) can be calculated.

\begin{displaymath}
P_{\textrm{in}} = \frac{\rho A}{d}
                = \frac{(350)(\num{1e-4})}{0.1}
                = \SI{350}{\milli\watt}
\end{displaymath}
